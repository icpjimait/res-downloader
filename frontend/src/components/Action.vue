<template>
  <div style="--wails-draggable:no-drag" class="grid grid-cols-3 gap-1.5">
    ...
    <n-icon
        v-if="row.Status === 'running' || row.Status === 'pending'"
        size="30"
        class="text-amber-500 dark:text-amber-400 bg-amber-500/20 dark:bg-amber-500/30 rounded-full flex items-center justify-center p-1.5 cursor-pointer hover:bg-amber-500/40 transition-colors"
        @click="action('cancel')"
    >
      <CloseOutline/>
    </n-icon>
    ...
    <NPopover placement="bottom" trigger="hover">
      ...
      <div class="flex flex-col">
        <!-- Removed the inner cancel button since it is now prominently displayed in the main action bar -->
